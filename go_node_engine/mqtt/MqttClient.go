...
// publishToBrokerFn allows overriding in tests
var publishToBrokerFn = func(topic string, payload string) {
	logger.InfoLogger().Printf("MQTT - publish to - %s - the payload - %s", topic, payload)
	if mainMqttClient == nil { // offline test mode
		return
	}
...
// (legacy function kept for backward compatibility if referenced elsewhere)
func publishToBroker(topic string, payload string) { publishToBrokerFn(topic, payload) }
...
func ReportServiceStatus(service model.Service) {
	...
	publishToBrokerFn("job", string(jsonmsg))
}
...
func ReportNodeInformation(node model.Node) {
	...
	publishToBrokerFn("information", string(data))
}
